/**
 * bulk-stock-keywords-10.mjs
 * 第10弾：第9弾の同シード × increaseKeyword: false（直接サジェスト）
 *         ＋ 第9弾で未採用の追加新根キーワード
 * ── 脂肪燃焼/断食/食べ方/ドリンク/運動/ボディ/新食材/目標 ──
 */
...
const SEEDS = [
  // ── 第9弾シードを increaseKeyword: false で再取得（直接サジェスト） ────
  ...
  { cat: 'ライフスタイル', seed: '1日 30分 ダイエット' },
  { cat: 'ライフスタイル', seed: 'ダイエット 記録 方法' },
  { cat: 'ライフスタイル', seed: 'ダイエット 習慣 朝' },
  ...
  { cat: 'ダイエット法', seed: 'リセット食 レシピ' },
  { cat: 'ダイエット法', seed: 'プチ断食 やり方' },
  { cat: 'ダイエット法', seed: '和食 ダイエット' },
  { cat: 'ダイエット法', seed: '地中海食 レシピ' },
...
function inferIntent(keyword) {
  if (/レシピ|食事|朝ごはん|朝食|昼食|ランチ|ディナー|夜ご飯|献立|コンビニ|外食|間食|断食|糖質|カロリー|たんぱく質|プロテイン|腸活|スープ|サラダ|鍋|おにぎり|パスタ|カレー|弁当|丼|料理|食材|食べ物|飲み物|ドリンク|スイーツ|お菓子|パン|低糖質|低カロリー|高たんぱく|ヘルシー|便秘|腸内|善玉菌|むくみ|作り置き|脂肪燃焼|代謝 食|ファスティング|チートデイ|食べ方|食べ順|食べ痩せ|食べても|食制限|糖質制限|ケトジェニック|スムージー|青汁|MCT|ルイボス|ハーブティ|デトックス|GI値|食物繊維|腸内フローラ|痩せ菌|置き換え|和食|地中海食/.test(keyword)) return 'meal';
  ...
  if (/下腹|脚痩せ|太もも|二の腕|お腹|背中|ウエスト|ヒップ|くびれ|顔痩せ|セルライト|リンパ|引き締め|細くする|内臓脂肪|皮下脂肪|体脂肪/.test(keyword)) return 'body-part';
  if (/続かない|やる気|始められない|モチベ|習慣|挫折|三日坊主|メンタル|自分に甘い|食べてしまう|ストレス 食べ|記録|失敗|成功|ズボラ/.test(keyword)) return 'mindset';
  if (/40代|50代|30代|20代|60代|更年期|産後|授乳|生理|PMS|妊活|妊娠|ブライダル|ホルモン/.test(keyword)) return 'life-stage';
  if (/ビタミン|コラーゲン|亜鉛|美肌|肌荒れ|アンチエイジング|髪|骨密度|貧血|冷え性/.test(keyword)) return 'beauty-health';
...
console.log('\n═══════════════════════════════════════════════════');
console.log(' 🌸 バルクキーワードストック 第10弾 美スリム研究所');
console.log(' ★ 第9弾同シード(false) + 新根キーワード追加');
console.log('═══════════════════════════════════════════════════');
...
for (let i = 0; i < SEEDS.length; i++) {
  ...
  // 第9弾シードの再取得は false、追加新規は true
  const isNewSeed = ['女性健康','美容インナー','ライフスタイル','ステージ','ダイエット法'].includes(cat);
  const increaseKeyword = isNewSeed ? true : false;
  ...
  try {
    ...
    totalCredit += credit;
    await logUsage('rakko', '/v1/suggest-keywords', credit, { seed, bulk: 10, increaseKeyword });
