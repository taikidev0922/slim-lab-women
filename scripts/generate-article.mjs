...
function renderSitemap(articles) {
  const staticUrls = ['/', '/blog/', '/about/'].map((url) => `  <url><loc>${config.siteUrl}${url}</loc><lastmod>${date}</lastmod><changefreq>${url === '/about/' ? 'monthly' : 'daily'}</changefreq><priority>${url === '/' ? '1.0' : '0.8'}</priority></url>`);
