...
import { articlePage, escapeHtml, slugify, ANALYTICS_TAG } from './lib/html.mjs';
...
function inferCategory(keyword) {
  if (/続かない|やる気|始められない|モチベ|習慣化|挫折|三日坊主|メンタル|自分に甘い|食べてしまう|ストレス|食欲|PMS|生理|更年期|睡眠|ビフォーアフター/.test(keyword)) {
    return 'マインド';
  }
...
async function updateIndexes() {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      articles.push({ slug, title, description, category, date: dateText, image: hasWebp ? 'image-01.webp' : hasPng ? 'image-01.png' : hasSvg ? 'image-01.svg' : 'image-01.png' });
    } catch {}
  }
